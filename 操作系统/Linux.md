...
硬盘最小的存储单位是扇区(Sector)，块(block)由多个扇区组成。文件存储在块中。块最常见的大小是4kb，约为8个连续的扇区组成（每个扇区存储约512字节）。一个文件可存储多个块，但一个块只能存储一个文件。

虽然，我们将文件存储在块中了，但是我们还需要一个空间开存储文件的**元信息**metadata：如果某个文件被分成几块、每一块在的地址、文件拥有者，创建时间，权限，大小等。这种**存储文件元信息的地址就叫做inode**，译为索引节点：i(index)+node。每个文件都有一个inode，存储文件的元信息。

可以使用 `stat` 命令查看文件的inode信息。每个inode都有一个号码，Linux/Unix操作系统不使用文件名来区分文件，而是使用inode号码区分不同的文件。
...
Linux支持多种文件类型，包括：**普通文件、目录文件、链接文件、设备文件、管道文件、Socket套接字文件** 等

- **普通文件**（-）：用于存储信息和数据，Linux用户可以根据访问权限对普通文件进行查看、更改和删除。比如声音、图片、PDF、源代码等
- **目录文件**（d ，directory file）：目录也是文件一种，用于表示和管理系统中的文件，目录文件包含一些文件名和子目录名。打开目录实际上就是打开目录文件
...
![img](https://mmbiz.qpic.cn/mmbiz_png/2Rp8GAOdFfBluq1Ir9g2VbkhqO2CAs8uvwD86ibIuiaBPw8HvibPKiaCWib2uw7PGiaC0E3V1rlcMlk3tYPXCTZUlgibQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/2Rp8GAOdFfBluq1Ir9g2VbkhqO2CAs8u6eJnaNAKT6LWN6Kp2NRC6cJv2QyXX5wNPazric8mpDjRnBh3nyOIHibg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)img

第一列的内容的信息解释如下：

![img](https://mmbiz.qpic.cn/mmbiz_png/2Rp8GAOdFfBluq1Ir9g2VbkhqO2CAs8uX1n9duHI5bRpibsKThzaMZO5dqnuYV8gSUVVKUlaSx6moRIsibfzaqQQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)img
...
![img](https://mmbiz.qpic.cn/mmbiz_png/2Rp8GAOdFfBluq1Ir9g2VbkhqO2CAs8uCOKGaAQQtRz8I9VzhRnLORwESHfJlgbvolUG11dmjLicJHncckoqBjw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)img
